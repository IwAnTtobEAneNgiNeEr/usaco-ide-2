// settings.js — Settings tab: compiler/judge config, AI config (key/baseUrl/model),
// compiler probe, AI connection test, and the old-tracker importer.
...
export function renderSettings(app) {
  ...
  app.el.settingsForm.innerHTML = `
    ...
    <div class="settings-group">
      ...
      <span id="compiler-status" class="hint"></span>
    </div>
    ...
    <div class="settings-group">
      ...
      <select id="set-accent-color" class="select">
        ${opt("blue", "Azure (Blue)", s.accentColor === "blue" || !s.accentColor)}
        ${opt("green", "Emerald (Green)", s.accentColor === "green")}
        ${opt("orange", "Amber (Orange)", s.accentColor === "orange")}
    ...
    <div class="settings-group">
      ...
      <input id="import-file" type="file" accept="application/json,.json" class="input" style="padding:6px" />
