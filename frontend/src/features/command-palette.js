...
function buildActions(app) {
  ...
  return [
    ...
    { icon: "⚡", label: "Flash Quiz (ôn lỗi sai)", kw: "flash quiz on tap trac nghiem loi sai recall", kbd: "", run: () => app.openFlashQuiz && app.openFlashQuiz() },
    { icon: "⚔️", label: "Boss cuối tuần", kw: "boss tuan weekend trum thu thach", kbd: "", run: () => app.goHome && app.goHome() },
    { icon: "🎓", label: "Bảo vệ AC (vấn đáp)", kw: "defense bao ve ac viva van dap hieu bai", kbd: "", when: has, run: () => app.openDefense && app.openDefense() },
